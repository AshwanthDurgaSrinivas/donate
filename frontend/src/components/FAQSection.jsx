                ...
                <div className="max-w-3xl mx-auto space-y-3">
                    {faqs.map((faq, i) => (
                        <motion.div key={i}
                            ...
                            className={`glass glass-hover border border-white/08 rounded-2xl overflow-hidden transition-all duration-300 ${open === i ? 'border-primary/30' : ''}`}>
                            <button
                            ...
                            >
                                ...
                                <motion.div animate={{ rotate: open === i ? 45 : 0 }} transition={{ duration: 0.25 }}
                                    className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ml-4 font-black text-lg transition-all ${open === i ? 'bg-primary/20 text-primary' : 'glass text-white/40'}`}>
                                    +
                                </motion.div>
                            </button>
                            <AnimatePresence initial={false}>
                                {open === i && (
                                    ...
                                    >
                                        <div className="px-5 pb-5 text-white/50 text-sm leading-relaxed border-t border-white/05 pt-4">
                                            {faq.a}
                                        </div>
                                    </motion.div>
                                )}
